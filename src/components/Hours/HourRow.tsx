...
export const HourRow: React.FC<HourRowProps> = ({
  ...
  const handleTagCycle = () => {
    ...
    const currentIndex = tags.indexOf((hourData?.tag || '') as TagCode);
    const nextTag = tags[(currentIndex + 1) % tags.length];
    onUpdate(nextTag, hourData?.note || '');
  };

  const handleNoteChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onUpdate((hourData?.tag || '') as TagCode, e.target.value);
  };
  ...
  return (
    <div className="flex items-center gap-1.5 py-1 px-1 border-b border-[var(--line)] hover:bg-[var(--teal-tint)]/40 transition-colors">
      ...
      {/* 4. زر النسخ السريع لأسفل (بعد زر التاقات) */}
      {!isLastHour && onCopyNext && (
        <button
          type="button"
          onClick={onCopyNext}
